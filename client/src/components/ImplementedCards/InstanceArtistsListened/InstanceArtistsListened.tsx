...
interface InstanceArtistsListenedProps extends ImplementedCardProps {}

export default function ArtistsListened({ className }: InstanceArtistsListenedProps) {
  const { interval, unit } = useSelector(selectRawIntervalDetail);
  const result = useAPI(api.instanceDifferentArtistsPer, interval.start, interval.end, Timesplit.all);
  const lastPeriod = useMemo(() => getLastPeriod(interval.start, interval.end), [interval]);
  const resultOld = useAPI(
    api.instanceDifferentArtistsPer,
    lastPeriod.start,
    lastPeriod.end,
    Timesplit.all,
  );
  ...
  if (!result || !resultOld) {
    return (
      <TitleCard title="Instance artists listened" className={className} fade>
